...
class MessageActor(Actor):
    async def msg_handler(self, msg=None):
        if not hasattr(self, "nsq_writer"):
            self.nsq_writer = await create_writer(
                host="127.0.0.1",
                port=4150,
...
class NsqKernelActor(KernelActor):
    async def msg_handler(self, msg=None):
        nsqd_tcp_addresses = msg.get("nsqd_tcp_addresses", ["127.0.0.1:4150"])
        max_in_flight = msg.get("max_in_flight", 200)
        topic = msg.get("topic")
        channel = msg.get("channel")
        reader = await create_reader(
            nsqd_tcp_addresses=nsqd_tcp_addresses, max_in_flight=max_in_flight
        )
